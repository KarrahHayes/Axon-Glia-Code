...
axr = Axis(r[1,1], xlabel = "Axon Radius (μm)", title = "Axon Size (n=371)", ylabel = "Probability", xscale = log10, xticks = [0.10, 0.316, 1.0, 3.16, 10.0])
hist!(axr, axonradius_um, color = RGBf(0.24, 0.7, 0.44), bins = exp.(LinRange(log(minval), log(maxval), 80)), normalization = :probability)
r 
...
# power law: y = b*x^a or log10(y) = alog10(x) + log10(b) in log form where
# just found that yint = log10(b) 
# therefore, log10(diameter) = a*log10(CV) + yint
# can solve for diameter by undoing the log: diameter = 10^(0.78*log10(CV) + yint)
# and thus, diameter = 10^yint * CV^0.78
# for roach, a = 0.78
